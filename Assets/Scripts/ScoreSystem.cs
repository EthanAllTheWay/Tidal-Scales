...
public class Score : MonoBehaviour
{
    ...
    void Update()
    {
        scoreText.text = "Score: " + PlayerPrefs.GetFloat("TotalScore");
    ...
    public void addScore() {
        fish = GameObject.FindGameObjectWithTag("Fish").GetComponent<Fish>(); //We Locate the fish who touched the note
        //If the diference between the song beat and the fish beat is smaller than 0.4 miliseconds, we got a perfect that multiplies the score x1.5
        if (Mathf.Abs(conductor.songPositionInBeats - fish.beatOfThisNote) < 0.4f) { //If the diference between the song beat and the fish beat is smaller than 0.4 miliseconds, we got a perfect that multiplies the score x1.5
            accuracy = 1.5f;
            Debug.Log("Perfect! "+ Mathf.Abs(conductor.songPositionInBeats - fish.beatOfThisNote));
        }
        else {
            accuracy = 1;
            Debug.Log("Good" + Mathf.Abs(conductor.songPositionInBeats - fish.beatOfThisNote));
        }
